...
4. `std::array<T, N>` (где создается в памяти)
	[[std-arrayT,N|std-arrayT,N]]
	
5. `std::vector<T>` (чем size отличается от capacity
6.  `std::deque<T>` 
7.  `std::list<T>`
8. Абстракции с использованием `std::stack<T>` и `std::queue<T>`
10. Деревья: `std::set<T>` и `std::map<K, V>`
11. Хеши: чем отличается `std::unordered_set<T>` от `std::unordered_map<K, V>`
---
...
---
## Не разобранные вопросы про код (TODO):
---
